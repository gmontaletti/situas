...
#'
#' @note This function is now a wrapper around \code{\link{prepare_territorial_maps}}.
...
#' }
prepare_comuni_maps <- function(situas_data = NULL,
                                 pfun = NULL,
                                 output_dir = "data",
                                 date = Sys.Date(),
                                 simplify = TRUE,
                                 tolerance = 0.001,
                                 keep_attributes = c("PRO_COM", "PRO_COM_T", "COMUNE",
...
#' Read Italian Municipalities Shapefile
#'
#' Internal helper function to read the municipalities shapefile from
#' extracted files or from the ISTAT zip archive.
#'
...
#' @keywords internal
#' @noRd
read_comuni_shapefile <- function(verbose = TRUE) {
  # Call generalized function for comuni
  read_territorial_shapefile(territorial_level = "comuni", verbose = verbose)
}
